...
const AboutSection = () => {
  return (
    <section id="sobre" className="bg-background py-24 md:py-36 relative overflow-hidden">
      ...
      <div className="absolute top-20 right-0 w-[400px] h-[400px] rounded-full bg-primary/[0.03] blur-3xl" />
      <div className="container max-w-6xl relative">
        <div className="grid md:grid-cols-[400px_1fr] gap-16 items-center">
          <AnimateOnScroll animation="animate-fade-in-left">
            <div className="relative">
              <div className="absolute -inset-5 rounded-[2rem] bg-secondary/60 -rotate-2" />
              <div className="absolute -inset-5 rounded-[2rem] gradient-primary opacity-[0.08] rotate-1" />
              <img
                src={professionalImg}
                alt="Foto do profissional especialista em movimento"
                className="relative rounded-3xl shadow-elegant w-full max-w-[400px] mx-auto object-cover aspect-[3/4] z-10"
                loading="lazy"
              />
              {/* Floating motivation badge */}
              <div className="absolute -bottom-4 -right-4 md:-right-8 rounded-2xl bg-background shadow-card-hover p-4 flex items-center gap-3 animate-float z-20 border border-border">
          ...
          <AnimateOnScroll animation="animate-fade-in-right" className="space-y-8">
            ...
            <div className="grid grid-cols-3 gap-4 pt-4">
              {stats.map(({ icon: Icon, value, label }) => (
                <div key={label} className="text-center p-5 rounded-2xl bg-section-alt shadow-card border border-border hover:shadow-card-hover hover:border-primary/20 transition-all duration-300">
            ...
            <Button variant="cta" size="lg" className="group mt-3" asChild>
